...
object TaskDAO extends DAO(classOf[Task], "task", TaskB.read) with datalayer.TaskDAO {

  override def saveSafe(c:Task) = {
    findAndReplace("_id" $eq c.id, TaskB.write(c), upsert=true).toRef
  }

  override def updateBody(t:Task) = updateAndFetch(
    query="_id" $eq t.id,
    update=$set("body" -> TaskBodyB.write(t.body))
  )

  override def byName(c:Ref[Course], n:String) = {
    for {
      cid <- c.refId
  ...
  override def byCourse(c:Ref[Course]) = {
    for {
      cid <- c.refId
