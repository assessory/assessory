...
object ShortTextQViews {
  ...
  def editShortTextAnswer(q:Question, a:ShortTextAnswer)(f: ShortTextAnswer => Unit):DHtmlContent = {
    def updateAnswer(ans:String):Unit = {
      f(a.copy(answer=Some(ans)))
    }

    <.div(^.cls := "form-group",
      <.textarea(^.cls := "form-control", ^.prop("value") := a.answer.getOrElse(""), ^.attr("rows") := 10,
        ^.on("input") ==> { _.inputValue.foreach(updateAnswer) }
      )
    )
  }

  def viewShortTextAnswer(q:Question, a:ShortTextAnswer):DHtmlContent = {
    <.div(^.cls := "form-group",
